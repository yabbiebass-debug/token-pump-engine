...
export const KpiBar = ({ data, fly, vault, onRun, onToggleAuto, running, activeStage, onWithdraw }) => {
      ...
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-4 sm:gap-6">
          ...
          <div data-testid="kpi-vault">
            <div className="text-[9px] uppercase tracking-[1.5px] text-dim font-bold flex items-center gap-1">SOL vault <span className="text-green text-[8px] bg-green/10 border border-green/30 px-1 rounded">LEDGER</span></div>
            <div className="flex items-center gap-2">
              <span className="font-display text-[15px] font-black text-green">{(vault?.available_sol ?? 0).toFixed(2)} SOL</span>
              <button onClick={onWithdraw} className="rounded bg-purple/20 border border-purple/50 px-2 py-0.5 text-[9px] text-purple hover:bg-purple hover:text-white uppercase font-bold transition-colors" data-testid="kpi-withdraw-btn">Withdraw</button>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <Btn variant="primary" onClick={onRun} disabled={running} data-testid="run-cycle-btn">
            <Play className={`h-3 w-3 ${running ? 'animate-spin' : ''}`} /> {running ? `Stage ${activeStage !== null ? activeStage + 1 : 8}/8…` : 'Run cycle'}
          </Btn>
        </div>
      </div>
    </header>
